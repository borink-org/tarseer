...
mod tests {
    ...
    fn blocked_walk_sink_bounds_reading_ahead() {
        ...
        for order in [tarseer::PartOrder::Walk, tarseer::PartOrder::Completion] {
            ...
            eprintln!(
                "2 walk threads, budget 1, order {order:?}, first sink blocked: {read_while_blocked} of {TOTAL} entries recorded (row window 4098)"
            );
            ...
            assert!(
                ...
                "{order:?}: {read_while_blocked} rows exceeded the bounded read-ahead window"
            );
        }
    }
}
